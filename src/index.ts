...
`,
   ...
   typeof requested.type === "undefined" ? CriticalError("type is required field. see docs to detail."): undefined;
    if(requested.type !== debTypes.isolated && requested.type !== debTypes.service && requested.type !== debTypes.normal && requested.type !== debTypes.symlink) {
   ...
   */
    ...
    candidate.postinst_payload = candidate.postinst_payload.replace(/\(PROJECT NAME\)/g, candidate.name);
    candidate.prerm_payload = candidate.prerm_payload.replace(/\(PROJECT NAME\)/g, candidate.name);
