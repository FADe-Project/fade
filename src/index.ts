...
import { args2config, FADeConfiguration, genRunbin, getFADeConfig, isSameOrUndef, stubCreateDeb, validate } from './utils';
...
async function main() {
    if(typeof args.help !== "undefined") {
        ...
        edit(args.path, args2config(args));
...
async function edit(path: string, requested: FADeConfiguration) {
    await validate(path);
